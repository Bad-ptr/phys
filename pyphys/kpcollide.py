...
class CollGridn( object ):
    def __init__( self, p0 = Vector2( ( 0.0, 0.0 ) ),
        ...
        self.p0 = p0
        self.p1 = p1
        self.pc = self.p0.vadd( self.p1 ).mul( 0.5 )
        self.bl = []
        self.numb = 0
        self.childs = []
        self.csubd( sl )
    ...
    def resize( self, p1 = Vector2( ( 100.0, 100.0 ) ) ):
        ...
        if self.childs != []:
            self.pc = self.p0.vadd( self.p1.vsub( self.p0 ).mul( 0.5 ) )
...
def wcollide( cg ):
    for p in cg.bl:
        if p.coord.x - p.size < cg.p0.x:
            p.coord.x = p.size
            p.vel.x = sqrt( p.vel.x**2 ) * 0.3
            #p.vel.x *= -0.2
        elif p.coord.x + p.size > cg.p1.x:
            p.coord.x = cg.p1.x - p.size
            p.vel.x = sqrt( p.vel.x**2 ) * -0.3
            #p.vel.x *= -0.2
        if p.coord.y - p.size < cg.p0.y:
            p.coord.y = p.size
            p.vel.y = sqrt( p.vel.y**2 ) * 0.3
            #p.vel.y *= -0.2
        elif p.coord.y + p.size > cg.p1.y:
            p.coord.y = cg.p1.y - p.size
            p.vel.y = sqrt( p.vel.x**2 ) * -0.3
            #p.vel.y *= -0.2
