...
class Sconst:
    ...
    def upd2b( self, fb, sb ):
        ...
        p = self.rl - dxm
        #tim = fb.invmass+sb.invmass
        #mv = dx.norm().mul( -p / tim )
        #fb.coord = fb.coord.vadd( mv.mul( fb.invmass ) )
        #sb.coord = sb.coord.vsub( mv.mul( sb.invmass ) )

        tim = fb.invmass + sb.invmass
        d1 = -p * ( fb.invmass / tim )
        d2 = p * ( sb.invmass / tim )
        fb.coord = fb.coord.vadd( dn.mul( d1 ) )
        sb.coord = sb.coord.vadd( dn.mul( d2 ) )
